...
class ClientPrefs {
	...
	public static var saveControls:FlxSave;
 
	// Null = both, False = Keyboard, True = Controller
	...
	public static function clearInvalidKeys(key:String) {
		...
		if (keyBind != null) while (keyBind.remove(NONE)) continue;
		if (gamepadBind != null) while (gamepadBind.remove(NONE)) continue;
	}
	...
	public static function bind() {
		final savePath = CoolUtil.getSavePath();

		if (saveControls == null) saveControls = new FlxSave();
		FlxG.save.bind('funkin', savePath);
		saveControls.bind('controls_v3', savePath);

		#if MERGE_PSYCH_DATA
		final psychPath = CoolUtil.getSavePath('', 'ShadowMario/PsychEngine');
		if (FlxG.save.isEmpty() && savePath != psychPath) {
			final oldSave = new FlxSave();

			if (oldSave.bind('funkin', psychPath)) {
				for (field in Reflect.fields(defaultData))
					if (Reflect.hasField(oldSave.data, field))
						Reflect.setField(FlxG.save.data, field, Reflect.field(oldSave.data, field));
			}
			if (oldSave.bind('controls_v3', psychPath))
				saveControls.mergeData(oldSave.data);
	...
	public static function saveSettings() {
		...
		saveControls.data.keyboard = keyBinds;
		saveControls.data.gamepad = gamepadBinds;
		saveControls.flush();
	...
	public static function loadPrefs() {
		...
		if (FlxG.save.data.framerate == null) {
			final refreshRate = FlxG.stage.application.window.displayMode.refreshRate;
		...
		if (saveControls != null) {
			var loadedKeys:Map<String, Array<FlxKey>> = saveControls.data.keyboard;
			if (loadedKeys != null) {
				for (control => keys in loadedKeys)
					if (keyBinds.exists(control)) keyBinds.set(control, keys);
			}

			var loadedBinds:Map<String, Array<FlxGamepadInputID>> = saveControls.data.gamepad;
			if (loadedBinds != null) {
				for (control => keys in loadedBinds)
