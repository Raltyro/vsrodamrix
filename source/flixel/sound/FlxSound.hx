...
class FlxSound extends FlxBasic
{
	...
	/**
	 * Internal tracker for a Lime audio source object.
	 */
	...
	@:privateAccess {
		if (_channel == null)
			(_source = (_channel = new SoundChannel()).__audioSource = new AudioSource()).gain = 0;
		else {
		...
		if (_source.buffer != null) _source.dispose();
		_source.buffer = _sound.__buffer;
		_source.init();
	...
	@:privateAccess {
		...
		_paused = false;
		_time = StartTime;
		if (_channel == null || !_channel.__isValid || _source.__backend == null) makeChannel();
	...
	function set_time(time:Float):Float @:privateAccess {
		...
		if (_channel != null && _realPitch > 0) {
			...
			if (!_channel.__isValid) {
				...
				_source.offset = 0;
				_source.currentTime = Std.int(time);
			}
			#end
		}
		return _time = time;
