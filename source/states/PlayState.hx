...
class PlayState extends MusicBeatState
{
	...
	private function generateSong(dataPath:String):Void
	{
		...
		try {
			if (songData.needsVoices) {
				...
				if (oppVocals != null) opponentVocals = FlxG.sound.load(oppVocals);
			}
		}
		catch(e:Dynamic) {}
	...
	function resyncVocals(resync:Bool = true):Void {
		...
		var stream = vocals.vorbis != null && opponentVocals.vorbis != null;
		if (!stream || resync) {
			if (stream) FlxG.sound.music.pause();
		...
		if (Conductor.songPosition <= opponentVocals.length) {
			...
			if (!stream || resync) {
				if (stream) opponentVocals.pause();
	...
	override public function update(elapsed:Float) {
		...
		if (FlxG.sound.music != null && FlxG.sound.music.playing && startedCountdown) {
		...
		if (startedCountdown && !paused) {
			var delta = FlxG.elapsed * 1000 * playbackRate;
			if (!startingSong && FlxG.sound.music.playing && songElapsed > 0) Conductor.songPosition = lastSongTime + Conductor.offset;
			else Conductor.songPosition += delta;

			if (!paused && !startingSong && (syncDebounce += elapsed) > 1) {
				syncDebounce = 0;

				var musicTime = FlxG.sound.music.time, vocalsTime = vocals.time, opponentVocalsTime = opponentVocals.time;
				var resync = vocals.loaded && Math.abs(vocalsTime - musicTime) > (vocals.vorbis == null ? 6 : 12);
				resync = resync || (opponentVocals.loaded && Math.abs(opponentVocalsTime - musicTime) > (opponentVocals.vorbis == null ? 6 : 12));
		...
		if (dunceNote != null) {
			var ogTime:Float = spawnTime * playbackRate;
			if (songSpeed < 1) ogTime /= songSpeed;

			var time:Float;
			while (unspawnNotes.length > 0) {
				time = ogTime;
				if (dunceNote.multSpeed < 1) time /= dunceNote.multSpeed;
				if (dunceNote.strumTime - Conductor.songPosition > time) break;
				notes.insert(0, dunceNote);
				dunceNote.spawned = true;

				var index:Int = unspawnNotes.indexOf(dunceNote);
				callOnLuas('onSpawnNote', [index, dunceNote.noteData, dunceNote.noteType, dunceNote.isSustainNote, dunceNote.strumTime]);
		...
		if (generatedMusic) {
			if (!inCutscene) {
				...
				if (notes.length > 0) {
					if (startedCountdown) updateNotes();
					else {
	...
	public dynamic function updateNotes() {
	...
	function set_health(value:Float):Float { // You can alter how icon animations work here {
		...
		healthBar.percent = newPercent = (newPercent != null ? newPercent : 0);

		iconP1.setState(newPercent < 20 ? 1 : (newPercent > 80 ? 2 : 0));
		iconP2.setState(newPercent < 20 ? 2 : (newPercent > 80 ? 1 : 0));
		return health;
	}

	function tryPause() {
		if (startedCountdown && canPause && callOnScripts('onPause', null, true) != LuaUtils.Function_Stop)
			openPauseMenu();
	}
